...
def wyswietl(usun, pobierz):
    ...
    Zwracane wartości: 0 lub pobrana wartość (lista - wartość (float), czy \
        faktura (int))
    """
    ...
    if usun:
        do_usuniecia = input(f"\nWybierz plik do usunięcia (wpisz [n n], jeśli\
...
def dane(czyFaktura):
    ...
    Argumenty: czy użytkownik chce dodać fakturę (jeśli nie, użytkownik dodaje\
          płatność)
    ...
    while True:
        ...
        kwota = re.sub("\,", ".", kwota)  # zamiana przecinka na kropkę
...
def przewalutowanie(kwota, waluta, data):
    ...
    if body.status_code == 200:  # sprawdzamy czy żądanie zostało poprawnie przetworzone
        ...
        print("\nBłąd! NBP nie opublikował kursów walut dla tego dnia. Proszę \
              wpisać wartość w PLN.\n")
        input("Naciśnij [Enter], aby kontynuować.")
        return 0
...
def oplacenie(faktura, platnosci):
    ...
    if wartosc > 0:
        print(f"\nFaktura nie została opłacona. Do dopłaty zostało:\
               {wartosc} PLN\n")
        input("Naciśnij [Enter], aby kontynuować.\n")
        return 0
...
def main():
    tryby = ['1', '2', '3', '4', '5', '6', '7', '0', '1 -h', '2 -h', '3 -h', '\
             4 -h', '5 -h', '6 -h', '7 -h', '0 -h']
    suma_faktura = 0
    platnosci = []
    nadpisanie = 0
    ...
    while True:
        tryb = input("Wybierz działanie:\
                     ...
                     \n[0] - Wyjdź\nWpisz -h po cyfrze (np. [1 -h]), aby\
                      wyświetlić pomoc dotyczącą działania.\n")
        ...
        if tryb in tryby:
            if tryb == "1":  # dodawanie i wpisywanie faktury
                ...
                while True:
                    ...
                    nadpisanie = input(f"Czy na pewno chcesz nadpisać obecną \
                                       fakturę opiewającą na {suma_faktura} \
                                        PLN? [y] - Tak, [n] - Nie: ")
            ...
            if tryb == "1 -h":
                print("\n[1] Pyta użytkownika o dane faktury (wartość, waluta,\
                       data). Następnie przeliczą jej wartość na PLN i \
                      załadowuje do systemu. W systemie może być jednocześnie \
                      aktywna tylko jedna faktura. Jeśli w systemie jest już \
                      aktywna faktura, użytkownik będzie musiał potwierdzić \
                      jej nadpisanie. Po wpisaniu danych, program spyta \
                      użytkownika czy zapisać fakturę do pliku.\n")
            ...
            if tryb == "2 -h":
                print("\n[2] Pyta użytkownika o dane płatności (wartość, \
                      waluta, data). Następnie przelicza jej wartość na PLN i \
                      załadowuje ją do systemu. Po wpisaniu danych, program \
                      spyta użytkownika czy zapisać płatność do pliku.\n")
            ...
            if tryb == "3 -h":
                print("\n[3] Pozwala na wybranie faktury lub płatności do \
                      załadowania z poprzednio utworzonego pliku.\n")
            ...
            if tryb == "4":  # usuniecie załadowanej platnosci lub faktury
                ...
                if usun_zaladowane == "1":
                    ...
                    wartosc = input(f"\nPłatność o jakiej wartości [PLN]\
                                     chcesz usunąć? (wpisz [n n], aby przerwać\
                                     akcję): {platnosci} ")
                    ...
                    if wartosc == "n n":
                        ...
                        print(f"\nPomyślnie usunięto płatność o wartości\
                               {wartosc}.\n")
                    else:
            ...
            if tryb == "5 -h":
                print("\n[5] Wyświetla listę plików utworzonych przez\
                       użytkownika oraz pozwala na ich odczytanie.\n")
            ...
            if tryb == "6 -h":
                print("\n[6] Pozwala na usunięcie plików utworzonych przez\
                       użytkownika.\n")
            ...
            if tryb == "7 -h":
                print("\n[7] Oblicza i wyświetla, ile zostało do opłacenia\
                       faktury.\n")
            ...
            if tryb == "0 -h":
                print("\n[0] Niepotrzebna funkcja. Po co ktoś chciałby zamykać\
                       ten świetny program? :)\n")
